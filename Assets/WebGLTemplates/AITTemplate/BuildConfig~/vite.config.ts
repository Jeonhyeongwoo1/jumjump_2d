...
//// USER_CONFIG_START ////
function aitUserAcceptsEncoding(
...
function aitContentEncodingCompatibilityPlugin(): Plugin {
  function createMiddleware() {
    return (
      ...
      if (requestPath.endsWith('.unityweb') || requestPath.endsWith('.br') || requestPath.endsWith('.gz')) {
        const contentEncoding = String(res.getHeader('Content-Encoding') || '').toLowerCase();
        if (
          (contentEncoding === 'br' || contentEncoding === 'gzip') &&
          !aitUserAcceptsEncoding(req, contentEncoding)
        ) {
  ...
  return {
    name: 'ait-content-encoding-compatibility',
    configureServer(server) {
...
const userConfig = defineConfig({
  plugins: [aitContentEncodingCompatibilityPlugin()],
});
//// USER_CONFIG_END ////
